app
		.controller(
				...
				function($scope, Labels, UiUtil, CrudUtil, $uiRouter,
						OperationUtil, $location, CurrentUser) {
					...
					$scope.preload = function() {
						...
						$scope.meetingCss = [ 'jalase1', 'jalase2', 'jalase3',
								'jalase4', 'jalase5', 'jalase6' ];
						...
						$scope.bossModel = UiUtil.getDefualtAutoCompleteModel(
								"Profile", "userName", "userName");

						$scope.tableModel.valueRenderer = function(element,
								column) {
							if ('workDay' == column) {
								return element.workDay + " - "
										+ element.dayDate;
							}
							return "";
						...
						$scope.tableModel.columnClicked = function(element,
							...
							if (columnValue.id == null) {
								...
								CrudUtil.getById('Meeting', columnValue.id)
					...
					$scope.saveOrUpdate = function() {
						...
						if (parseInt($scope.modelData.startTime) >= parseInt($scope.modelData.endTime)) {

							alertWarning(Labels.Warning.correctlyFild);
							return;

						} else if (!$scope.modelData.id) {
							CrudUtil
