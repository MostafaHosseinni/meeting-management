app
		.controller(
				...
				function($scope, $location, Labels, CrudUtil, OperationUtil) {
					...
					$scope.preload = function() {
						...
						$scope.newPasswordOperations = [
								...
								OperationUtil.getDefaultIconOperation(
										'fa fa-eye-slash', function() {
					...
					$scope.changePassword = function() {
						if (!$scope.modelForm.$valid) {
							...
							alertWarning(Labels.Error.notEqualPassword);
						} else {
							CrudUtil
									.customPostService($scope.entityName,
											$scope.modelData, 'changePassword')
									.then(
											function(data) {
												localStorage.paH = md5($scope.modelData.newPassword
														+ Labels.app);
